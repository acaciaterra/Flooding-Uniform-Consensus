...
auto ultima_msg_rodada = std::make_unique<tmsg>();
int ultimo_propose;
...
int token, N, timestamp = 0, s, ultimo_nodo, decided = -1, rodada = 1;
...
void verify(int recebeu){
	...
	for(int i = 0; i < N; i++){
		if(status(nodo[i].id) == 0){
			cont++;
			for(int j = 1 ; j < nodo[recebeu].delivered[rodada - 1].size(); j++){

					// printf("******************* %d %d o valor de j eh: %d\n", nodo[recebeu].delivered[rodada - 1][j], nodo[i].idr, j);
				 if(nodo[recebeu].delivered[rodada - 1][j] == nodo[i].idr){
					cont1++;
					//nada a fazer
				}
			}
	...
	// printf("VETOR DE PROPOSTAS\n");
	// for(int i = 0; i < nodo[recebeu].proposal_set.size(); i++){
	// 	printf("%d ", nodo[recebeu].proposal_set[i]);
	// }
	// printf("\n");

	// printf("----------------- %d %d\n", cont, cont1);
	//se todos os corretos ja receberam e ainda nao foi decidido
	if(cont == cont1 && decided == -1){
		if(rodada == N){
			//menor valor em proposal set
			std::sort(nodo[recebeu].proposal_set.begin(), nodo[recebeu].proposal_set.end());
			decided = nodo[recebeu].proposal_set[0];
			printf("[DECIDE] todos os nodos sem-falha decidiram na rodada %d pelo valor %d\n", rodada, decided);
		} else {
			int tam;
			rodada++;
			printf("\n---------------------- Inicio da rodada %d -------------------\n\n", rodada);

			// for(int i = 1; i < nodo[recebeu].delivered[rodada - 2].size(); i++){
			// 	nodo[recebeu].delivered[rodada - 1].push_back(nodo[recebeu].delivered[rodada - 2][i]);
			// 	printf("ADICIONOU O VALOR: %d\n", nodo[recebeu].delivered[rodada - 2][i]);
			// }
			//faz novo broadcast (?)
			...
			mensagens.push_back(tmsg{'T', "nova rodada: " + std::to_string(rodada), nodo[recebeu].idr, timestamp});
			tam = mensagens.size()-1;
			// std::cout << "Mensagem armazenada::::::::::::::::::" << mensagens[mensagens.size()-1].m << "\n";

			ultima_msg_rodada->type = mensagens[tam].type;
			ultima_msg_rodada->m.assign(mensagens[tam].m);
			ultima_msg_rodada->idorigem = mensagens[tam].idorigem;
			ultima_msg_rodada->timestamp = mensagens[tam].timestamp;
			//incrementa o timestamp sempre que gera uma nova mensagem
			timestamp++;

			schedule(broadcast_rodada, 1.0, recebeu);
		}

	}
}

//nessa funcao verificamos se todo mundo recebeu a mensagem de "nova rodada"
void verify_rodada(int recebeu){
	// printf("ENTROU NA VERIFY RODADAAAAAAAAA\n");
	// printf("ID ORIGEM DA ULTIMA MENSAGEM ENVIADA %d \n", ultima_msg_rodada->idorigem);
	int cont = 0, cont1 = 0;
	for(int i = 0; i < N; i++){
		if(status(nodo[i].id) == 0){
			cont++;
			// printf("SIZE: %d\n", nodo[i].delivered[rodada - 1].size());
			for(int j = 1 ; j < nodo[i].delivered[rodada - 1].size(); j++){
				// printf(">>>>>>>> %d ---- %d  j: %d\n", nodo[i].delivered[rodada - 1][j], nodo[i].idr, j);
				 if(nodo[i].delivered[rodada - 1][j] == ultima_msg_rodada->idorigem){
					//  printf("CONT: %d\n", cont1);
					cont1++;
					//nada a fazer
				}
...
void freceive(int enviou, int recebeu){
	...
	|| ultima_msg->timestamp > nodo[recebeu].last[ultima_msg->idorigem].timestamp){
		...
		// printf("Status do nodo: %d\n", status(nodo[ultima_msg->idorigem].id));
			...
			nodo[recebeu].proposal_set.push_back(ultimo_propose);
			// printf("Foi adicionado o valor: %d pro nodo %d\n", ultima_msg->idorigem, recebeu);
			nodo[recebeu].delivered[rodada - 1].push_back(ultima_msg->idorigem);

			// printf("VETOR DELIVERED\n");
			// for(int i = 0; i < nodo[recebeu].delivered[rodada - 1].size(); i++){
			// 	printf("%d ", nodo[recebeu].delivered[rodada - 1][i]);
			// }
			// printf("\n");
	...
	verify(ultima_msg->idorigem);
}

void freceive_rodada(int enviou, int recebeu){
	int k, etp;
	bool res;

	if(nodo[recebeu].last[ultima_msg_rodada->idorigem].type == 'N'
	|| ultima_msg_rodada->timestamp > nodo[recebeu].last[ultima_msg_rodada->idorigem].timestamp){
		//last_i[source(m)] <- m
		//Atualiza o last do nodo atual com a mensagem que acabou
		//de ser recebida do outro nodo
		nodo[recebeu].last[ultima_msg_rodada->idorigem].type = ultima_msg_rodada->type;
		nodo[recebeu].last[ultima_msg_rodada->idorigem].m.assign(ultima_msg_rodada->m);
		nodo[recebeu].last[ultima_msg_rodada->idorigem].idorigem = ultima_msg_rodada->idorigem;
		nodo[recebeu].last[ultima_msg_rodada->idorigem].timestamp = ultima_msg_rodada->timestamp;
		//deliver(m)
		std::cout <<"[DELIVER] mensagem \"" << nodo[recebeu].last[ultima_msg_rodada->idorigem].m << "\" recebida do nodo " << enviou <<" foi entregue para a aplicacao pelo processo " << recebeu << " no tempo {" << time() << "}\n\n";
		// printf("Nodo: %d\n", nodo[ultima_msg_rodada->idorigem].idr);
		// printf("Status do nodo: %d\n", status(nodo[ultima_msg_rodada->idorigem].id));
			//
			// nodo[recebeu].proposal_set.push_back(ultimo_propose);
			// printf("Foi adicionado o valor: %d pro nodo %d\n", ultima_msg_rodada->idorigem, recebeu);

			nodo[recebeu].delivered[rodada - 1].push_back(ultima_msg_rodada->idorigem);
		...
		if(status(nodo[ultima_msg_rodada->idorigem].id) != 0){
			// std::cout <<"Fazendo novo broadcast para os vizinhos sem-falha... " << "\n";
			schedule(broadcast_rodada, 1.0, recebeu);
			// broadcast(*ultima_msg_rodada);
			return;
		}
	}
	//encontrar identificador do cluster s de i que contem j
		...
		for(int j = 0; j < tests[i].size; j++){
			for(int k = 1; k <= etp - 1; k++){
				if(tests[i].vector[j] == recebeu && tests[i].etapa[j] == k && status(nodo[i].id) == 0){
					...
					//retorna true, caso exista
					res = std::any_of(nodo[recebeu].ack_set.begin(), nodo[recebeu].ack_set.end(), [i, enviou] (auto const &elem){
						return elem.idorigem == enviou && elem.iddestino == nodo[i].idr && !(elem.m.m.compare(ultima_msg_rodada->m));
					});
					//if <j,k,m> nao pertence ack_set(i)
					// j = enviou, k = ff_neighbor (aqui eh o nodo[i])
					if(!res){
						nodo[recebeu].ack_set.insert(tset{enviou, nodo[i].idr, *ultima_msg_rodada});
						printf("[RBCAST] processo %d envia mensagem para processo %d\n", recebeu, nodo[i].idr);
						freceive_rodada(recebeu, nodo[i].idr);
					}
				}
			}
		}
	...
	checkacks(nodo[enviou], *ultima_msg_rodada);
	verify_rodada(ultima_msg_rodada->idorigem);
}
...
void print_init(){
	printf ("=====================================================================\n");
	printf ("         Flooding Uniform Consensus using Reliable Broadcast\n");
...
void print_end(int r, int n){
	...
	printf("\nOs nodos decidiram pelo valor: %d\n", decided);
...
//-------variáveis dos eventos passados por arquivo---------
	...
	for (i = 0; i < N; i++) {
		...
		nodo[i].proposal_set.resize(0);
		nodo[i].delivered.resize(N);
		for (int j = 0; j < N; j++){
			nodo[i].state[j] = 0;
			...
			nodo[i].delivered[j].resize(1);
			nodo[i].delivered[j][0] = -1;
		}
	 }
...
//-----------------------a cada evento, recalcula o vetor tests----------------------------------
			...
			if(ultima_msg->idorigem == token && status(nodo[token].id) == 0){
				...
				// nodo[token].proposal_set.push_back(ultimo_propose);
				nodo[token].delivered[rodada - 1].push_back(token);
			...
			ultimo_propose = valor;
			//armazena em mensagens todas as mensagens ja enviadas pelo broadcast,
			//contendo o id de origem e o timestamp (unico para cada mensagem)
			//--- transforma a string em um tmsg ---
			mensagens.push_back(tmsg{'T', "propose "+ std::to_string(valor) + " rodada 1", nodo[token].idr, timestamp});
			tam = mensagens.size()-1;
			// std::cout << "Mensagem armazenada::::::::::::::::::" << mensagens[mensagens.size()-1].m << "\n";
			...
			case broadcast_rodada:
			//if source(m) == i
			if(ultima_msg_rodada->idorigem == token && status(nodo[token].id) == 0){
				//isso aqui não vai gerar look infinito não? HAHAHAHAHAHAHAHAHAH
				while(resb){
					//existe algum elemento em ack_set que respeite <i, *, last_i[i]>?
					//retorna true, caso exista
				...
				//last_i[i] = m
				nodo[token].last[token].type = ultima_msg_rodada->type;
				nodo[token].last[token].m.assign(ultima_msg_rodada->m);
				nodo[token].last[token].idorigem = ultima_msg_rodada->idorigem;
				nodo[token].last[token].timestamp = ultima_msg_rodada->timestamp;
			...
			for(int i = 0; i < N; i ++){
				for(int j = 0; j < tests[i].size; j++){
					if(tests[i].vector[j] == token){
						...
						// printf("i: %d\n", i);
						printf("[RBCAST] processo %d envia mensagem para processo %d\n", token, nodo[i].idr);
						freceive_rodada(token, nodo[i].idr);
					}
				}
			}
			break;
